...
### Testimi i API-së
Pasi serveri të jetë ndezur:

1. Krijo një përdorues me domain-in e lejuar:
   ```bash
   curl -X POST http://localhost:8000/auth/register \
     -H "Content-Type: application/json" \
     -d '{
       "email": "student@universiteti.edu",
       "password": "sekret123",
       "full_name": "Student Test",
       "role": "student"
     }'
   ```
2. Merre token-in JWT për hyrje:
   ```bash
   curl -X POST http://localhost:8000/auth/login \
     -H "Content-Type: application/x-www-form-urlencoded" \
     -d "username=student@universiteti.edu&password=sekret123"
   ```
   Kopjo `access_token` nga përgjigja.
3. Bëj kërkesa të sigurta duke shtuar header-in `Authorization: Bearer <token>` (p.sh. për të parë profilin):
   ```bash
   curl http://localhost:8000/auth/me \
     -H "Authorization: Bearer <token>"
   ```

### Mbushja e të dhënave demo
Për një tur të shpejtë, ekzekuto skriptin e seed-it pas krijimit të tabelave:

```bash
python -m backend.app.seeds
```

Skriptit është idempotent: nëse e ekzekuton disa herë nuk do të krijojë dublikata. Ai shton një kurs demonstrues, module, leksione, detyra, quiz-e, njoftime, mesazhe dhe përdorues `admin@example.com`, `instructor@example.com` dhe `student@example.com` me fjalëkalimin `Password123!`. Sigurohu që `ALLOWED_EMAIL_DOMAIN` të jetë `example.com` ose modifiko email-at në skript sipas domain-it tënd.
